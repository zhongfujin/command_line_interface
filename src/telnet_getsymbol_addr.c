...
unsigned int debug_init_list()
{
	...
	SLL_LIST_INIT(g_so_name_list->so_name_list);
...
uintptr_t ld_find_address(const char *sym_name)
{
	...
	SO_NODE *sllnode = SLL_LIST_HEAD(g_so_name_list->so_name_list,offsetof(SO_NODE,node));
    while(sllnode)
    {
        ...
        sllnode = SLL_LIST_NEXT(sllnode->node, offsetof(SO_NODE,node));
		//printf("hah %s\n",sllnode->map.pathname);
    }
...
unsigned int so_name_add_node(SO_NAME_LIST *list,struct ld_maps *map)
{
	...
	memset(so_node,0,sizeof(SO_NODE));
    memcpy(&so_node->map,map,sizeof(struct ld_maps));
    if(!SLL_LIST_INSERT_TAIL(list->so_name_list,so_node->node))
    {
...
unsigned int get_maps_so_list(pid_t pid)
{
    ...
    while(fgets(maps,bufsz,fp) != NULL)
    {
        ...
        {
            temp_node = (SO_NODE *)SLL_LIST_TAIL(g_so_name_list->so_name_list,offsetof(SO_NODE,node));
            if(temp_node != NULL)
            {
