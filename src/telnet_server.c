...
unsigned int telnet_ser()
{
    ...
    while(1)
    {
        ...
        count = select(max_fd + 1,&rfdset,NULL,NULL,NULL);
        if(count <= 0)
        {
            continue;
        }
...
void *shell_thread_func(void *arg)
{
    ...
    int i = 0;
	int complete = 0;
	CLI_CMD_LINE_LIST *root_cmd_line_list = NULL;
    ...
    while(1)
    {
		...
		if(cmd_line[0] == '?')
		{
			complete = strlen(cmd_buf);
			cli_cmd_complete_cmd(tel_mgr->fd_conn,cmd_buf,0);
			if(complete == strlen(cmd_buf))
			{
				write_certain_bytes(tel_mgr->fd_conn,"\r\n",strlen("\r\n"));
		...
		if(cmd_line[0] == '\t')
		{
			...
			write_certain_bytes(tel_mgr->fd_conn,DEBUG_SHELL,strlen(DEBUG_SHELL));*/
			complete = strlen(cmd_buf);
			cli_cmd_complete_cmd(tel_mgr->fd_conn,cmd_buf,1);
			if(complete == strlen(cmd_buf))
			{
				write_certain_bytes(tel_mgr->fd_conn,"\r\n",strlen("\r\n"));
				write_certain_bytes(tel_mgr->fd_conn,DEBUG_SHELL,strlen(DEBUG_SHELL));
...
CMD_OVER:
            ...
            if(cmd_buf[0] != '\r')
            {
                ...
                redirect_io(tel_mgr);
                write_certain_bytes(tel_mgr->fd_conn,cmd_buf,strlen(cmd_buf));
                root_cmd_line_list = cli_get_cli_cmd_line_list();
                int index = cli_find_cmd_line_index(root_cmd_line_list,cmd_buf,"debug");
                printf("\nindex is %d\n",index);
