...
unsigned int tel_log_init()
{
	...
	SLL_LIST_INIT(g_tellog_reglist->reg_list);
...
TEL_LOG_REG_NODE *tel_add_log_reg_node(TEL_LOG_REG_INFO *reg_info)
{
    ...
    SLL_LIST_INSERT_TAIL(tel_log_reg_list->reg_list,reg_node->tel_log_reg_node);
...
TEL_LOG_REG_NODE *tel_log_search_node(unsigned int module_id)
{
    ...
    {
        node = SLL_LIST_GET_INDEX(g_tellog_reglist->reg_list,i,offsetof(TEL_LOG_REG_NODE,tel_log_reg_node));
